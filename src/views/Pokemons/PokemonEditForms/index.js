...
const PokemonAddForms = () => {
  const { setReloadTable, currentPokemon, setCurrentPokemon, setIsEditing } =
  ...
  const UpdatePokemon = async (data, e) => {
    await PokemonApi.updatePokemon(currentPokemon.id, data);
    setCurrentPokemon([]);
    setReloadTable(true);
  };
  ...
  setValue("name", currentPokemon.name);
  setValue("image", currentPokemon.image);
  setValue("attack", currentPokemon.attack);
  setValue("defense", currentPokemon.defense);

  return (
    <>
      <form onSubmit={handleSubmit(UpdatePokemon)}>
        ...
        <div className="columns-1 text-center  mg-y-5">
          <button type="submit">
            <img alt="Img" className="ico" src={SavIicon} />
            Actualizar
          </button>
          ...
          >
            <img alt="Img" className="ico" src={CloseIcon} />
            Cancelar
          </button>
        </div>
      </form>
